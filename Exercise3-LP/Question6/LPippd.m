function [x, info, mu, lambda, iter, history] = LPippd(g, A, b, x)
...
history.rL = []; history.rA = []; history.s = [];
...
while ~Converged && (iter<maxit)
    ...
    history.rL(iter) = norm(rL, inf);
    history.rA(iter) = norm(rA, inf);
    history.s(iter) = abs(s);
